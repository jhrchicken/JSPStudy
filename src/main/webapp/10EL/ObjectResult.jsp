...
<body>
	<!-- 영역에 저장된 속성에 EL로 접근할 때는 속성명이 중복되지 않는다면 EL내장객체 없이 속성명 만으로 출력할 수 있다.
	즉 requestScope.personObj.name 형태로 기술하지 않아도 된다.
	
	"속성명.멤버변수" 형태로 기술하면 클래스에 선언된 getter()를 자동으로 호출하여 값을 출력한다.
	즉 아래 문장은 getter()를 호출하므로 Person 클래스에서 getName()을 주석처리하면 에러가 발생한다.
	...
	// 출력 시에는 멤버변수에 직접 접근할 수 없으므로 getter()를 사용한다.
	out.print("이름 : " + person.getName());
	// 이런 모든 정차를 EL은 자동으로 처리해준다.
	%>
	...
	<ul>
		...
		하지만 EL에서는 자동으로 숫자형식으로 변환되므로 아래와 같이 즉시 연산할 수 있다. -->
