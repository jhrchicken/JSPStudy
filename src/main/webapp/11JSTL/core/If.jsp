...
<body>
	...
	<!-- JSTL의 if태그는 else 구문이 별도로 없으므로 if문과 반대의 조건을 만들어서 두개의 if태그를 사용한다. -->
	<!-- Java에서는 비교연산자 == 과 eqauls()가 서로 다르지만, EL에서는 eq를 통해 값에 대한 비교와 문자열에 대한 비교를 모두 할 수 있다. -->
    ...
    만약 틀렸다면 "아이디/비번을 확인하세요"라고 출력한다. EL과 JSTL의 if태그만을 이용해서 구현하시오. -->
    <form method="get">
		아이디 : <input type="text" name="user" /><br/>
		패스워드 : <input type="text" name="pass" /><br/>
		<input type="submit" value="로그인" />
	</form>
	<!-- 최초 실행 시에는 파라미터가 없는 상태이므로 아무 내용도 출력되면 안된다. 전송된 파라미터가 있을 때만 실행되도록 한다. -->
