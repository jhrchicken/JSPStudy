...
24년 7월 현재까지 사용에는 전혀 문제가 없다. 호환성을 위해 남겨져있는 상태이다. -->
<%-- @ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" --%>
<!-- JSTL 최신 버전에서 사용하는 지시어
jakarta로 시작하는 패키지명을 사용한다. -->
...
<body>
	<!-- set태그
	: 변수를 선언할 때 사용 JSP의 setAttribute() 메서드와 동일한 기능을 수행한다. 4가지 영역에 새로운 속성을 추가한다.
	...
	value에는 일반값, EL, 표현식을 모두 사용할 수 있다. 또한 태그 사이에 value를 삽입할 수도 있다. -->
	...
	<h4>자바즈 생성 1 - 생성자 사용</h4>
	<!-- JSTL은 JSP 코드이므로 value 속성에 기술할 때 인스턴스 생성을 위한 더블쿼테이션이 겹쳐지는 경우가 발생하게 된다.
	이때는 value를 싱글쿼테이션으로 감싸서 겹쳐지지 않게 처리해야 한다. -->
	...
	<!-- 빈 인스턴스를 먼저 생성한 후 target, property 를 통해 멤버변수의 값을 설정한다. 이때 자바빈의 setter()가 사용된다. -->
	<h4>자바즈 생성 2 - target, property 사용</h4>
