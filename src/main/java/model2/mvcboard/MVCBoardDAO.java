...
public class MVCBoardDAO extends DBConnPool {
	...
	// 게시물의 개수를 카운트하는 메서드. 검색어가 있는 경우 where절을 동적으로 추가한다.
	...
	public int insertWrite(MVCBoardDTO dto) {
		int result = 0;
		try {
			/* 쿼리문에서 사용하는 시퀀스는 모델1 게시판에서 생성한 것을 그대로 사용한다.
			...
			psmt.setString(4, dto.getOfile()); // 원폰파일명
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			rs = psmt.executeQuery();
			// count() 함수는 항상 결곽가 있으므로 조건문 없이 호출한다.
			rs.next();
			if(rs.getInt(1) == 0) {
