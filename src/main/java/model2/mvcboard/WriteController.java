...
글쓰기 페이지로 진입할 때는 get 방식의 요청이고, 전송(submit)을 눌러 폼값을 서버로 전송할 때는 post 방식으로 요청하게 된다. */
public class WriteController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 첨부파일이 정상적으로 등록되어 원본파일이 반환되었다면, 원본 파일명과 저장된 파일 이름 설정
		if (originalFileName != "") {
			// 파일명 변경 (파일명을 "날짜_시간.확장자" 형식으로 변경)
			String saveFileName = FileUtil.renameFile(saveDirectory, originalFileName);
			
			// 원본파일명과 서버에 저장된 파일명을 DTO에 저장
