...
public class BoardDAO extends JDBConnect {
	...
	/* 생성자 호출 시 application 내장객체를 매개변수로 받은 후 부모클래스의 생성자를 호출할 때 재사용한다.
	부모클래스는 web.xml에 등록된 컨텍스트 초기화 파라미터를 얻어와서 DB에 연결한다. */
	...
	public int selectCount(Map<String, Object> map) {
		...
		try {
			...
			System.out.println("게시물 수를 구하는 중 예외가 발생했습니다.");
			e.printStackTrace();
		}
	...
	/* 작성된 게시물을 인출하여 반환한다.
	특히 반환값은 여러개의 레코드를 반환할 수 있고, 순서를 보장해야 하므로 List를 사용한다. */
	public List<BoardDTO> selectList(Map<String, Object> map) {

		/* List 계열의 컬렉션 생성한다.
		이때 타입매개변수는 board 테이블을 대상으로 하므로 BoardDTO로 설정한다. */
		List<BoardDTO> bbs = new Vector<BoardDTO>();

		/* 레코드 인술을 위한 select 쿼리문 작성
		최근 게시물이 상단에 출력돼야 하므로 일련번호의 내림차순으로 정렬한다. */
	...
	public BoardDTO selectView(String num) {
		...
		/* 내부조인(inner join)을 통해 member 테이블의 name 컬럼까지 select 한다. */
